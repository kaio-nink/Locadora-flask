...
def cadastrarCliente(con, id_cliente, nome, cpf):
    cur = con.cursor()
    cur.callproc('addCliente', [id_cliente, nome, cpf])
    con.commit()
    con.close()
...
def cadastrarProduto(con, id_produto, nome, preco_unitario, qtd):
    cur = con.cursor()
    cur.callproc('addProduto', [id_produto, nome, preco_unitario, qtd])
    con.commit()
    con.close()
...
def cadastrarVenda(con, id_compra, id_cliente, id_produto, qtd_venda):
    ...
    cur = con.cursor()
    cur.callproc('addVenda', [id_compra, id_cliente, id_produto, data_venda, qtd_venda])
    con.commit()
